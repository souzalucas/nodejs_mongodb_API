...
router.post('/auth', async (req, res) => {
    ...
    try {
        ...
        if (err) res.status(500).send({error: 'Erro ao buscar o usuário!'});
    }
});
...
400 - Bad request.
401 - Unauthorized -- Autenticação (Caráter temporário).
403 - Forbidden -- AUTORIZAÇÃO, tem caráter permanente.
404 - Not Found.

500 - Internal server error.
501 - Not implemented - a API não suporta essa funcionalidade.
503 - Service Unavailable - a API executa essa operação, mas no momento não está indisponível.
